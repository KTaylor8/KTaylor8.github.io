...
class network():
    ...
    def feedforward(self, inList):
        ...
        """
        # for layer in range(len(self.layerSizes)):  # num layers #debugging
        layer = 0
        ...
        for receivingN in range(self.layerSizes[layer]):  # debugging
            ...
            layerI = 0  # hidden layer; nums based on w and b lists
            # for each input that the receiving neuron receives:
            for inNum in range(len(inList)):
                outList = []  # outputs from all hidden neurons for one input put into network
                # for neuron in range(self.layerSizes[layerI+1]): don't think needed
                hiddenW = self.w[layerI][receivingN][inNum]
                hiddenB = self.b[layerI][receivingN][0]
                print(
                ...
                neuronOut = neuronOutArr[0]
                outList.append(neuronOut)
                inList = outList
                # print('hiddenOutputList: ', hiddenOutputList)  # debugging

            layerI += 1  # output layer
        rawExpOut = inList[0]
