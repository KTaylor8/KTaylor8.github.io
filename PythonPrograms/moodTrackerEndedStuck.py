...
# it thinks everything is positive with only the custom data and it thinks sad and bad are neutral unless they're together (very similar to moodTracker.py)
...
def graphSentiments():
    ...
    Note: no parameters, so no doctest
    """
    # # old, not edited since originally worked:
    # with open("moodTrackerData.json", "r+") as dataFile:
    ...
    # # the x-ticks seem okay if you have at least 5 entries
    # plt.ylabel('Sentiment')
    # plt.xlabel('Entry Number')
    # plt.title('Sentiment vs Entry Number Graph')
    # plt.show()

    # new; the tick labels don't line up with the actual coordinates:
    try:
        with open("moodTrackerData.json", "r+") as dataFile:
            entries = json.load(dataFile)
            entriesCount = [entry for entry in range(len(entries))]
        ...
        for i in range(1, len(entriesCount)+1):  # shift over 1
            xTicksList.append(i)
        graphLine = plt.plot(xTicksList, sentimentList, 'b-')
        plt.ylabel('Sentiment')
        plt.xlabel('Entry Number')
        plt.title('Sentiment vs Entry Number Graph')
