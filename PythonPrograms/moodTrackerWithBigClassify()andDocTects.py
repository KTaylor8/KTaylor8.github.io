...
def classifyResp(userResp):
    ...
    Your response was overall positive.
    ...
    for (string, sentiment) in posResps + negResps:
        ...
        filteredWords = [el.lower() for el in string.split() if len(el) >= 3]
    ...
    # ‘trainingData’ should contain the labeled feature sets: a list of tuples for each resp each containing the feature dictionary and the sentiment string for each resp.
    trainingData = [
...
def main():
    ...
    while repeat == "yes":
        ...
        if userResp == "delete":
            ...
            storeMood(classifyResp(userResp))
